...
const FormLayout = ({formTitle, headerArr, children, buttonLabel, submitFunc}:{formTitle:string, headerArr: any, children: any, buttonLabel: any, submitFunc: any}) => {
    return (
        <Box sx={{ width: '100%', flexWrap: 'wrap', textAlign: 'left', ml: 'auto', mr: 'auto', height: 'fit-content',  }}>
            ...
            {/* Form Header */}
            {headerArr.length > 0 ? <Box sx={{display: 'flex', flexWrap: 'wrap',width: '100%', justifyContent: 'space-around', textAlign: 'left', mb: 4, bgcolor: 'rgba(0, 0, 0, 0.1)', p:2}}>
                {
            ...
            <Box sx={{ width: '100%', left: 0, bottom: 0, textAlign: 'right', pl: 4}}>
                <Button variant="contained" size="medium" sx={{ml: 1}} onClick={submitFunc}>{buttonLabel}</Button>
            </Box>
        </Box>
    )
}
